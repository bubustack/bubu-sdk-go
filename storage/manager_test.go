...
func TestNewManager_FileStore(t *testing.T) {
	...
	err := os.Setenv("BUBU_STORAGE_PROVIDER", "file")
	if err != nil {
		t.Fatalf("Setenv() error = %v", err)
	}

	err = os.Setenv("BUBU_STORAGE_PATH", tmpDir)
	if err != nil {
		t.Fatalf("Setenv() error = %v", err)
	}
	defer func() {
		err = os.Unsetenv("BUBU_STORAGE_PROVIDER")
		if err != nil {
			t.Fatalf("Unsetenv() error = %v", err)
		}
		err = os.Unsetenv("BUBU_STORAGE_PATH")
		if err != nil {
			t.Fatalf("Unsetenv() error = %v", err)
		}
	}()
...
func TestNewManager_NoProvider(t *testing.T) {
	_ = os.Unsetenv("BUBU_STORAGE_PROVIDER")
	_ = os.Unsetenv("BUBU_STORAGE_PATH")
...
func TestNewManager_FileProviderMissingPath(t *testing.T) {
	...
	_ = os.Setenv("BUBU_STORAGE_PROVIDER", "file")
	_ = os.Unsetenv("BUBU_STORAGE_PATH")
...
func TestNewManager_CustomMaxInlineSize(t *testing.T) {
	_ = os.Setenv("BUBU_MAX_INLINE_SIZE", "1024")
...
func TestNewManager_InvalidMaxInlineSize(t *testing.T) {
	_ = os.Setenv("BUBU_MAX_INLINE_SIZE", "not-a-number")
	defer func() { _ = os.Unsetenv("BUBU_MAX_INLINE_SIZE") }()
...
func TestNewManager_FileStore_InvalidPath(t *testing.T) {
	_ = os.Setenv("BUBU_STORAGE_PROVIDER", "file")
	_ = os.Setenv("BUBU_STORAGE_PATH", "/this/path/does/not/exist")
	defer func() { _ = os.Unsetenv("BUBU_STORAGE_PROVIDER") }()
	defer func() { _ = os.Unsetenv("BUBU_STORAGE_PATH") }()
...
func TestNewManager_S3Store_NoCredentials(t *testing.T) {
	...
	_, err := NewManager(ctx)
	if err == nil {
		t.Error("NewManager() should return an error when S3 provider is selected" +
			"but no credentials or region are configured")
	}
}
