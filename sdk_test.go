...
func (m *MockBatchEngram[C, I]) Process(ctx context.Context, execCtx *engram.ExecutionContext, inputs I) (*engram.Result, error) {
...
func TestRun_Success(t *testing.T) {
	...
	os.Setenv("BUBU_STEPRUN_NAME", "test-step-run")
	defer os.Unsetenv("BUBU_STORY_NAME")
	defer os.Unsetenv("BUBU_STEPRUN_NAME")
...
func TestHandleResultAndPatchStatus(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := handleResultAndPatchStatus(ctx, mockSM, mockK8s, execCtxData, tt.result, tt.processErr, false)
